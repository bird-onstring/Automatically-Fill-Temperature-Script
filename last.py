...
#百度ai平台
APP_ID = '27809333'
API_KEY = '8jGctUZUNOGONjZNzEbPe00U'
SECRET_KEY = 'Xkzy1MQ1CxO9A20dvwMFr5oR6ccaMNe1'
...
f = open('user_list.txt', mode='r', encoding='utf-8')  # 打开txt文件，以‘utf-8'编码读取
...
def print_log(text):
    ...
    f = open(syslog, "a")
    text = '%s\t%s\n' % (time.strftime('%Y.%m.%d %H:%M:%S', time.localtime(time.time())), str(text))
    print(text, file=f)
    f.close()
...
def write_tem(get_dict):
    ...
    driver.set_page_load_timeout(20)
    try:
        driver.find_element(By.XPATH, "//*[@id='form']/div[10]/div/div/div[2]/div/div/label[1]").click()
        
        
        time.sleep(0.5)
        driver.find_element(By.XPATH,"//*[@id='form']/div[11]/div/div/div[2]/div/div/label[1]/div").click()
        time.sleep(0.5)

        driver.find_element(By.XPATH, "//*[@id='form']/div[19]/div/div/div[2]/div/div/label[1]").click()
        time.sleep(0.5)
        driver.find_element(By.XPATH, "//*[@id='form']/div[19]/div/div/div[2]/div/div/label[1]/div").click()
        time.sleep(0.5)
  
        driver.find_element(By.XPATH,"//*[@id='ssl']/input").send_keys(get_dict['qinshi'])
        time.sleep(0.5)
        
        driver.find_element(By.XPATH,"//*[@id='qs']/input").send_keys(get_dict['louhao'])
    ...
    msg_from = '2304475012@qq.com' # 发送方邮箱
    passwd = 'yxtctiljrtqidjbc'  #就是上面的授权码
 
    to= ['2304475012@qq.com'] #接受方邮箱
